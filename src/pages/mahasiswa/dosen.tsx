...
function DosenCard({ data }: any) {
    const dummyPhoto = 'https://source.unsplash.com/random/?person'

    return (
        <Link to={'/dosen/1'} className="bg-white rounded-lg p-4 flex flex-col items-center gap-3">
...
export default function MahasiswaDosen() {
    const [filter, setFilter] = useState('semua')
    const dummyDosens = dummyPartners
    ...
    return (
        <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-3 gap-3">
                {dummyDosens && dummyDosens.map((dosen, index) => (
                    <DosenCard data={dosen} key={index} index={index} />
                ))}
            </div>
        </div>
    )
